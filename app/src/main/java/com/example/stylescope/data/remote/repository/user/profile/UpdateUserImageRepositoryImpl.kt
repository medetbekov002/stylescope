...
class UpdateUserImageRepositoryImpl(private val api: ApiService) : UpdateUserImageRepository {
    override fun updateUserImage(model: UpdateUserImageModel): Flow<Either<String, UpdateUserImageModel>> =
        makeNetworkRequest { api.updateUserImage(model = model.toData()).toDomain() }
}